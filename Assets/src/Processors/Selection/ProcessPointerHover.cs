...
namespace ThePathfinder.Processors
{
  ...
  public sealed class ProcessPointerHover : ProcessorInput, ITick
  {
    ...
    public void Tick(float delta)
    {
      ...
      || player.GetButtonUp(ActionId.Selection_Confirm)) { return; }
        ...
        foreach (var bounds in _bounds)
        {
          ...
          if (cPointerBounds.value.bounds.IntersectRay(r))
          {
            ...
            if (bounds.Has<Selectable>() && !bounds.Has<SelectionCandidate>())//if ent can be selected but currently isn't
              bounds.Get<SelectionCandidate>() = new SelectionCandidate(commander);
          }
          else
          {
            if (bounds.Has<PointerHover>()) bounds.Remove<PointerHover>();
            if (bounds.Has<Selectable>() && bounds.Has<SelectionCandidate>()) bounds.Remove<SelectionCandidate>();
          }
        }
      }
    }
