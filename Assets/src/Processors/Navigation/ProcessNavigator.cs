    ...
    sealed class ProcessNavigator : Processor, ITick
    {
        ...
        public void Tick(float delta)
        {
            foreach (var unit in _navigatorsWithDestination)
            {
                ...
                unit.Get<SpeedMod>().value =
                    approachingFinalWaypoint && unit.DestinationQueueComponent().destinations.Count == 0
                        ? math.sqrt(distanceToWaypoint / nextWaypointDistance)
                        : 1f;
